...
% ========================================================================
% The forward modeling routine for the DTM1 model using the full second
% order bases
...
sag=zeros(totkenar*4+totnode+totyuzey*3,2);
...
klerv2=zeros(1,30);  %all non-phi terms
for i=1:6
    if(kler(i)>0)
    klerv2(i)=kler(i);
...
klerv1=zeros(1,10); %all phi terms
klerv1(1:4)=kler3;
for i=1:6
if(kler(i)>0)
...
iszerov2=length(find(klerv2<0));  %all non-phi terms
    ...
    if(iszerov2==0)
        ...
        rr=repmat(klerv2',[1 30]); %row indices
        cc=rr'; %column indices;

        ix1(sayac1+1:sayac1+900)=rr(:);
        iy1(sayac1+1:sayac1+900)=cc(:);
        iv1a(sayac1+1:sayac1+900)=RR(:);
        iv1b(sayac1+1:sayac1+900)=FF(:);
        sayac1=sayac1+900;
    else
    ...
    if(iszerov1==0)
        ...
        rr=repmat(klerv1',[1 10]); %row indices
        cc=rr'; %column indices;
    ...
    if(iszerov1==0 && iszerov2==0)
        rr=repmat(klerv1',[1 30]); %row indices
        cc=repmat(klerv2,[10 1]); %column indices

        ix3(sayac3+1:sayac3+300)=rr(:);
        iy3(sayac3+1:sayac3+300)=cc(:);
        iv3(sayac3+1:sayac3+300)=DD(:);
        sayac3=sayac3+300;
    ...
    if(iszero~=0) %If it does not lie on a boundary, use this block
        ...
        for i=1:length(ke)
            ...
            sag(kler2(nke2)+totkenar*3,:)=sag(kler2(nke2)+totkenar*3,:)+sag_local3(nke2,:);
         end
    ...
    if(iszero2~=0)
        nke=find(kler>0); % these will remain
        nke2=find(kler2(1:8)>0); % these will remain
        ke=find(kler2(1:8)<0); % these will be removed from the system
...
R1=sparse(ix1,iy1,iv1a,totkenar*3+totyuzey*3,totkenar*3+totyuzey*3); % double curl
M1=sparse(ix1,iy1,iv1b,totkenar*3+totyuzey*3,totkenar*3+totyuzey*3); %
D1=sparse(ix3,iy3,iv3,totnode+totkenar,totkenar*3+totyuzey*3); %
...
for jj=1:size(recv,1)
    ...
    klern=[klern kler(1:6)+totnode]+totkenar*3+totyuzey*3;
