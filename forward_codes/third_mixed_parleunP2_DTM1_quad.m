...
% x134=(pn1+pn3+pn4)/3;
% x234=(pn2+pn3+pn4)/3;
% ekval2=[x123';x124';x134';x234'];
% else
% ekval2=valklst(ekyuz,:);
% end
% ekval=[ekval1;ekval2];
% 
% end
...
% for i=1:4
% if(kler2(i)>0)
% klerv1(i+16)=kler2(i)+totnode+totkenar*2;
% else
% klerv1(i+16)=kler2(i);
% end
% end
...
iszerov2=length(find(klerv2<0));  %tüm non-phi
    ...
    if(iszerov1==0 && iszerov2==0)
        rr=repmat(klerv1',[1 45]); %row nolar
        cc=repmat(klerv2,[10 1]); %col nolar
        
        ix3(sayac3+1:sayac3+450)=rr(:);
        iy3(sayac3+1:sayac3+450)=cc(:);
        iv3(sayac3+1:sayac3+450)=DD(:);
        sayac3=sayac3+450;
...
D1=sparse(ix3,iy3,iv3,totnode+totkenar,totkenar*3+totyuzey*6+totel*3); % 
...
for jj=1:size(recv,1)
    ...
    % klern=[klern kler(1:12)+totnode kler2(1:4)+totnode+totkenar*2]+totkenar*3+totyuzey*6+totel*3;

    klern=EL(ii,12:15)-totkenar;
    klern=[klern kler(1:6)+totnode]+totkenar*3+totyuzey*6+totel*3;
...
% save('mixedthirdDTM1.mat','R1','M1','roa','faza','Zler','Tler');
...
rr=load('miensopustUngauged1.mat');
...
zxy2(i,1)=rr.zxy{i,2}(46,1);
zyx2(i,1)=rr.zyx{i,2}(46,1);
...
zxy2=abs(zxy2).^2./(mu*2*pi./E(:,4));
zyx2=abs(zyx2).^2./(mu*2*pi./E(:,4));
...
subplot(4,2,3);semilogx(ff,zxy,'x');
hold on; semilogx(ff,squeeze(zxy2),'o');title('Zxy');
% subplot(4,2,4);semilogx(ff,zxyf,'x');
% hold on; semilogx(ff,squeeze(faza(:,41,3))+180,'o');title('Zxy');


subplot(4,2,5);semilogx(ff,zyx,'x');
hold on; semilogx(ff,squeeze(zyx2),'o');title('Zyx');



ezxy2=abs((zxy-squeeze(zxy2))./zxy)*100;
ezyx2=abs((zyx-squeeze(zyx2))./zyx)*100;



figure;
semilogx(ff,ezxy2,'+');hold on;semilogx(ff,ezyx2,'v');hold on;
legend('Zxy error','Zyx error');xlabel('frequency');ylabel('%');
