...
%%%%%P3 NEDEN ÇALIŞMIYOR GALİBA ÇALIŞMAMASI LAZIM
...
sag=zeros(totkenar*2+totnode,2);
...
iszerov2=length(find(klerv2<0));  %tüm non-phi
    ...
    if(iszerov2==0)
        ...
        rr=repmat(klerv2',[1 12]); %row nolar
        cc=rr'; %col nolar;
        
        ix1(sayac1+1:sayac1+144)=rr(:);
        iy1(sayac1+1:sayac1+144)=cc(:);
        iv1a(sayac1+1:sayac1+144)=RR(:);
        iv1b(sayac1+1:sayac1+144)=FF(:);
        sayac1=sayac1+144;
    else
    ...
    if(iszerov1==0 && iszerov2==0)
        rr=repmat(klerv1',[1 12]); %row nolar
        cc=repmat(klerv2,[4 1]); %col nolar
        
        ix3(sayac3+1:sayac3+48)=rr(:);
        iy3(sayac3+1:sayac3+48)=cc(:);
        iv3(sayac3+1:sayac3+48)=DD(:);
        sayac3=sayac3+48;
...
R1=sparse(ix1,iy1,iv1a,totkenar*2,totkenar*2); % double curl
M1=sparse(ix1,iy1,iv1b,totkenar*2,totkenar*2); % 
D1=sparse(ix3,iy3,iv3,totnode,totkenar*2); % 
...
for jj=1:size(recv,1)
    ...
    duzkenar(i+6,:)=(duzkose(i1)*G(i2,:)+duzkose(i2)*G(i1,:))/(6*Ve)*le(i);
...
save('fullfirstDTM1.mat','R1','M1','roa','faza','Zler','Tler');
