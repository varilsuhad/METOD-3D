...
iszerov2=length(find(klerv2<0));  %tüm non-phi
    ...
    if(iszerov2==0)
        ...
        rr=repmat(klerv2',[1 30]); %row nolar
        cc=rr'; %col nolar;
    ...
    if(iszerov1==0 && iszerov2==0)
        rr=repmat(klerv1',[1 30]); %row nolar
    ...
    if(iszero2~=0)
        nke=find(kler>0); % bunlar kalacak
        nke2=find(kler2(1:8)>0); % bunlar kalacak
        ke=find(kler2(1:8)<0); % bunlar dizeyden düşecek
...
bsag=sag(1:totkenar*4+totyuzey*3+totnode,:);


[x1,res1,relres1] = gpbicgV4M9(Amatris1,bsag(:,1),MM,10^-9,200);
[x2,res2,relres2] = gpbicgV4M9(Amatris1,bsag(:,2),MM,10^-9,200);
...
% bsag=sag(1:totkenar*3+totyuzey*3,:);
bsag=sag;
...
save('fullsecondDTM1.mat','R1','M1','roa','faza','Zler','Tler');
